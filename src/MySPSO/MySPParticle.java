 ...
 * Added sumBestPosition
 * @author Hernani Chantre<hdchantre@gmail.com>
 */
public abstract class MySPParticle {

	private static final double NULL = 0;
	/** Sum number of bestPosition[] */
	double sumBestPosition;
	 ...
	 */
	public MySPParticle(int dimension) {
		allocate(dimension);
	}
	 ...
	 */
	public MySPParticle(MySPParticle sampleParticle) {
	...
	public void init(double maxPosition[], double minPosition[], double maxVelocity[], double minVelocity[]) {
		for (int i = 0; i < position.length; i++) {
			...
			position[i] = Math.round((maxPosition[i] - minPosition[i]) * Math.random() + minPosition[i]);
	...
	public void setFitness(double fitness, boolean maximize) {
		...
		//boolean value = VectorGenerator.setConstraint1(bestPosition);
	...
	public String toString() {
		String str ="";
		this.sumBestPosition = 0;


		if (bestPosition != null  ) {

			str +=  bestFitness+ "\t"+ this.getFactor()  ;
			for (int i = 0; i < bestPosition.length; i++)
				this.sumBestPosition += bestPosition[i] ;
			// added to compute total number of reliable components created
			str += "\t" + this.sumBestPosition
					;
		}
		/*if (position != null) {
			str += "\n\tPosition:\t";
		...
		if (bestPosition != null  ) {
			...
			double [] v = new double [MySPDataset.numSubSystem];



			for (int l = 0 ; l< v.length ; l++)
			{ 
				for (int j = 0 ;j < MySPDataset.numComp; j++)
				{

					v[l] += bestPosition[MySPDataset.numComp*l+j];
